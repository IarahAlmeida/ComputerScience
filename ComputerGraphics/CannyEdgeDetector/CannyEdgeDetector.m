...
function B = GetDirections(A)
    ...
    for i = 1  : r
        for j = 1 : c
            ...
            if ((A(i, j) >= 337.5) && (A(i, j) <= 360) || (A(i, j) >= 0 ) && (A(i, j) < 22.5) || (A(i, j) >= 157.5) && (A(i, j) < 202.5))
                B(i, j) = 0;
...
function B = Hysteresis(A)
    ...
    [r, c] = size(A);
    B = zeros(r, c);
    % até 10% do valor máximo da imagem será considerado um pixel fraco,
    % isto é, definitivamente não borda
    ThreshL = 0.1 * max(max(A));
    % acima de 20% do valor máximo da imagem será considerado um pixel
    % forte, isto é, definitivamente borda
    ThreshH = 0.2 * max(max(A));
    % classifica pixels em bordas (0) ou não bordas (1) e julga os demais de acordo
    % com seus vizinhos
    for i = 1  : r
        for j = 1 : c
            % se definitivamente não borda, é transformado em um pixel branco
            if (A(i, j) <= ThreshL)
                B(i, j) = 255;
            % se definitivamente borda, é transformado em um pixel preto
            elseif (A(i, j) >= ThreshH)
                B(i, j) = 0;
            % caso nenhum dos casos acima mas possua um vizinho definitivamente
            % borda, também é considerado borda e transformado em um pixel preto
            elseif (A(i + 1, j) >= ThreshH || ...
                    A(i - 1, j) >= ThreshH || ...
                    A(i, j + 1) >= ThreshH || ...
                    A(i, j - 1) >= ThreshH || ...
                    A(i + 1, j + 1) >= ThreshH || ...
                    ...
                    A(i - 1, j + 1) >= ThreshH)
                B(i, j) = 0;
            % caso não tenha pelo menos um vizinho definitivamente borda, é
            % considerado não borda e transformado em um pixel branco
            else
                B(i, j) = 255;
            end
        end
